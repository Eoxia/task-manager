...
<div id="wpeo-task-option" class="wpeo-display-option">
	...
	<ul class="wpeo-end-time">
		...
		<li>
			<form action="<?php echo admin_url('admin-ajax.php'); ?>" method="POST">
				 <!-- For admin-ajax -->
			 	<?php wp_nonce_field( 'ajax_update_due_date_' . $element->id ); ?>
				<input type="text" class="isDate" name="due_date" value="<?php echo !empty( $element->option['date_info']['due'] ) && !empty( max( $element->option['date_info']['due'] ) ) ? max( $element->option['date_info']['due'] )['date'] : current_time( 'Y-m-d' ); ?>" />
				<input type="hidden" name="task_id" value="<?php echo $element->id; ?>" />
