...
<ul class="wpeo-point-comment wpeo-point-comment-<?php echo $time->id; ?>" data-id="<?php echo $time->id; ?>">
	...
	<li class="action">
		...
		<?php if ( get_current_user_id() == $time->author_id ): ?>
			<span data-nonce="<?php echo wp_create_nonce( 'ajax_get_point_time_' . $time->id ); ?>" class="dashicons dashicons-edit wpeo-point-time-edit-btn"></span>
			<span data-nonce="<?php echo wp_create_nonce( 'ajax_delete_point_time_' . $time->id ); ?>" class="dashicons dashicons-no wpeo-send-point-time-to-trash"></span>
		<?php endif; ?>
	</li>
