...
window.eoxiaJS.taskManagerFrontend.comment.beforeLoadComment = function( triggeredElement, response ) {
	...
	jQuery( '.wpeo-project-task .point .point-toggle .fa.fa-arrow-down' ).toggleClass( 'fa-arrow-right fa-arrow-down' );

	return true;
};

window.eoxiaJS.taskManagerFrontend.comment.loadedFrontComments = function( triggeredElement, response ) {
	triggeredElement.closest( '.point' ).find( '.point-toggle .fa' ).toggleClass( 'fa-arrow-right fa-arrow-down' );
