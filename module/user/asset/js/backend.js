...
var wpeo_user = {
	...
	open_list: function( event, element ) {
		...
		wpeo_user.current_task_id = object_id;
	...
	open_list_owner: function( event, element ) {
		...
		wpeo_user.hide_no_active( event );
		wpeo_user.current_task_id = task_id;

		/** On rajoute une class à l'objet pour ne pas reéxécuter cette requête AJAX */
		bloc_user.find( '.wpeo-user-owner' ).addClass( 'wpeo-confirm-user' );
	...
	close_list_owner: function( event ) {
